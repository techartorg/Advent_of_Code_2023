...
class Solver(ProblemSolver):
    def __init__(self):
        super(Solver, self).__init__(6)

        self.testDataAnswersPartOne = [288]
        self.testDataAnswersPartTwo = [71503]

    def ProcessInput(self, data=None):
        """
        Match up race times and distances and return them as tuples

        :param str data: the raw input data

        :returns list[(int, int)]: the processed race results
        """
        if not data:
            data = self.rawData
        
        times = [int(i) for i in data.splitlines()[0].split(':')[-1].split(' ') if i.strip()]
        distances = [int(i) for i in data.splitlines()[1].split(':')[-1].split(' ') if i.strip()]

        return [(times[i], distances[i]) for i in range(len(times))]

    def findMinMaxValues(self, time, distance):
        """
        Find the minimum and maximum amounts of time you'd need to hold down 
        the button to get to the input distance
        
        :param int time: How long the race lasts
        :param int distance: The distance we must exceed within the given time

        :returns int, int: the minimum and maximum amounts of time we can hold the button down to win the race
        """
        # figure out the minimum and maximum amount of time we need to hold the button down for
        minValue = 0
        maxValue = 0

        # brute force would have you start from the top and bottom and work your way toward the middle
        # to figure out min and max values
        i = 0
        while not minValue and i < time:
            i += 1
            testDistance = i * (time - i)
            if testDistance > distance:
                minValue = i
            
        
        i = time
        while not maxValue and i > 0:
            i -= 1
            testDistance = i * (time - i)
            if testDistance > distance:
                maxValue = i
    ...
    def SolvePartOne(self, data=None):
        """
        Determine the product of the margin of error for how long the boat's button 
        can be held down in order to achieve the desired results in a given race

        :param list[(int, int)] data: the race times and distances

        :return int: the product of the margins of error in all of the races
        """
        if not data:
            data = self.processed
        ...
        for time, distance in data:
            minValue, maxValue = self.findMinMaxValues(time, distance)

            marginOfError = (maxValue - minValue) + 1
    ...
    def SolvePartTwo(self, data=None):
        ...
        # dehydrate and reconstitute the input data, since we need it in a different
        # form for part 2

        time = int(''.join([str(time) for time, distance in data]))
        distance = int(''.join([str(distance) for time, distance in data]))
