...
def calculate_focusing_power(lens_boxes):
    focusing_power = 0

    for box_num in sorted(lens_boxes.keys()):
        # print(f'{lens_boxes[box_num]}')
        for box_idx, item in enumerate(lens_boxes[box_num]):
            result = (box_num + 1) * (box_idx + 1) * item[1]
            focusing_power += result

            print(f'\t[{lens_boxes[box_num][box_idx][0]} {lens_boxes[box_num][box_idx][1]}] : {box_num + 1} (box {box_num}) * {box_idx + 1} (slot) * {item[1]} (focal length) = {result} ({focusing_power})')

    return focusing_power


def calculate_hash_value(character, current_value):
    if DEBUG: print(f'\tThe current value is: {current_value}')
    current_value += ord(character)
    if DEBUG: print(f'\t\tThe ascii code for {character} is: {ord(character)}')
    current_value = current_value * 17
    if DEBUG: print(f'\t\tThe current value * 17 is: {current_value}')
    current_value = current_value % 256
    if DEBUG: print(f'\t\tThe current value becomes: {current_value % 256}')
...
def parse_data(raw_data, part_two = False):
    ...
    if not part_two:
        ...
        for datum in raw_data.split(','):
            focal_length = ''
            regex_result = re.search('([a-zA-Z]*)([-|=])(\d*)', datum)

            if regex_result[3]:
                focal_length = regex_result[3]

            data.append( [ regex_result[1], # label
                           regex_result[2], # operator
...
def main(raw_data, part_two = False):
    results = []
    data = parse_data(raw_data, part_two)
    lens_boxes = defaultdict(deque)
    ...
    for datum in data:
        ...
        for char in label:
            box_num = calculate_hash_value(char, box_num)
        ...
        if part_two:
            ...
            if datum[2]:
                lens_length = int(datum[2])

            # if lens_boxes[box_num]: print(f'{lens_boxes[box_num][0][0]} {lens_boxes[box_num][0][1]} -> {datum[0]} {datum[2]}')
            # Remove lens
            if operator == '-':
                a = deque([x for x in lens_boxes[box_num] if x[0] != label])

                for idx, lens in enumerate(lens_boxes[box_num]):
                    if lens[0] == label:
                        lens_boxes[box_num].rotate(-idx)
                        lens_boxes[box_num].popleft()
                        lens_boxes[box_num].rotate(idx)
                        # for x in sorted(lens_boxes.keys()):
                            # if lens_boxes[x]: print(f'\tBox {x}: {lens_boxes[x]}')
                        break
                print(lens_boxes[box_num] == a)

            # Add lens
            else:
                existing_lens = [x for x in lens_boxes[box_num] if x[0] == label]

                if len(existing_lens) > 1:
                    assert f'Too many lenses of the same label in box {box_num}'

                elif existing_lens:
                    idx = lens_boxes[box_num].index(existing_lens[0])
                    lens_boxes[box_num][idx] = [label, lens_length]
                    # for x in sorted(lens_boxes.keys()):
                        # if lens_boxes[x]: print(f'\tBox {x}: {lens_boxes[x]}')
                    abab = 1

                # No lenses that match. Add to the end
                else:
                    lens_boxes[box_num].append([label, lens_length])
        ...
        results.append(box_num)

    if not part_two:
        print(f'\nThe sum of the results is {sum(results)}')
    else:
        focusing_power = calculate_focusing_power(lens_boxes)
        print(f'\nThe focusing power from the resulting lens configuration is {focusing_power}')



if __name__ == "__main__":
    input = r"D:\Projects\Advent_of_Code\2023\day_15_input.txt"
    raw_data = []

    with open(input, "r") as input_file:
        raw_data = input_file.readline( ).rstrip()
