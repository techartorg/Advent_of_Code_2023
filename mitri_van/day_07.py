...
CARD_STRENGTH_MAPPING = [ 'A', 'K', 'Q', 'J', 'T', '9', '8', '7', '6', '5', '4', '3', '2' ]
TEST_DATA = [ '32T3K 765',
              'T55J5 684',
              'KK677 28',
              'KTJJT 220',
...
class Card():
    ...
    def __init__(self, hand, bid, rank = 1, card_type=0, part_two=False):
        self.id = hand
        self._hand = ''
        self.bid = int(bid)
        self.rank = rank
    ...
    def __repr__(self):
        return f"<{self.__class__.__name__} {self.id} ({self.bid},{self.rank}) {self.get_type()}>"

    def __str__(self):
        return f"<{self.__class__.__name__} {self.id} ({self.bid},{self.rank}) {self.get_type()}> {self._hand}"

    def __eq__(self, other):
        if self.type == other.type:
            self_val = self._hand[idx]
            other_val = other._hand[idx]
            if self.part_two and self_val == 'B': self_val = '1'
            if self.part_two and other_val == 'B': other_val = '1'
    ...
    def __gt__(self, other):
        if self.type == other.type:
            for idx in range(0,5):
                ...
                if int(self_val, 16) > int(other_val, 16):
                    return True
                elif int(self_val, 16) < int(other_val, 16):
                    return False
                else:
                    pass # Values are equal, continue checking
            return False
        return self.type > other.type
    ...
    def _set_hand(self):
        hand = self.id.replace('A', 'E').replace('K', 'D').replace('Q', 'C').replace('J', 'B').replace('T', 'A')
        self._hand = hand
    ...
    def _set_type(self):
        ...
        for char in self._hand:
            if self.part_two and char == 'B':
                wildcard += 1

            elif char in cache.keys():
                cache[char] += 1
        ...
        if self.part_two:
            ...
            if wildcard != 5:
                cache[sorted_cache[-1][0]] += wildcard
            else:
                self.type = 7

        # Evaluate cards
        for card in cache.keys():
            # Five of a kind
            if cache[card] == 5:
                self.type = 7
                break
...
def set_rank(cards,  part_two = False):
    idx = 0
...
def parse_data(raw_data, part_two = False):
    ...
    for datum in raw_data:
        hand, bid = datum.split()
        result.append(Card(hand, bid, part_two=part_two))

    return result


def main(raw_data, part_two = False):
    cards = parse_data(raw_data, part_two)
    set_rank(cards, part_two)

    [print(f'{card} : {card.winnings}') for card in sorted(cards)]

    winnings = sum([card.winnings for card in cards])

    print(f'\nThe total winnings are {winnings}')


if __name__ == "__main__":
    input = r"D:\Projects\Advent_of_Code\2023\day_07_input.txt"
    raw_data = []
